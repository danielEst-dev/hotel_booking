...
const validateCreateRoomRequest = (form) => {
	const formShape = {
		room_number: yup.string().max(10).matches(/^[A-Za-z1-9][A-Za-z0-9\-]*$/, 'Invalid room number').required(),
...
const validateUpdateRoomRequest = (form) => {
	const formShape = {
		id: yup.number().integer().min(1).required(),
		room_number: yup.string().max(10).matches(/^[A-Za-z1-9][A-Za-z0-9\-]*$/, 'Invalid room number').optional(),
		room_type: yup.string().optional(),
